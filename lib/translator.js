...
defineMapping(['mode'],               [['metadata', 'labels', 'cs-mode']]);
defineMapping(['praetor', 'leader_eligible'], [['metadata', 'labels', 'cs-mode']]);
defineMapping(['praetor', 'leader'],          [['metadata', 'labels', 'cs-mode']]);
defineMapping(['last_sync'],          [['status', 'conditions', 2, 'lastHeartbeatTime']]);
defineMapping(['state'],              [['status', 'conditions']]);
defineMapping(['start_time'],         [['metadata', 'creationTimestamp']]);
defineMapping(['address', 'public'],  [['metadata', 'labels', 'public-ip']]);
defineMapping(['address', 'private'], [['metadata', 'labels', 'private-ip']]);
...
defineMapping(['metadata', 'kubernetes', 'version'], [['status', 'nodeInfo', 'kubeletVersion']]);
...
defineConversion(['state'], (conditions) => {
    const ready_status = _.find(conditions, { type: 'Ready' });

    return ready_status.status === 'True' ? 'Ready' : 'NotReady';
});

defineConversion(['praetor', 'leader_eligible'], (v) => {
    return v === 'leader' ? true : false;
});

// todo - support multiple leaders
defineConversion(['praetor', 'leader'], (v) => {
    return v === 'leader' ? true : false;
});
...
function csStatusFromK8SStatus(pod) {
    if(_.has(pod, 'status.containerStatuses[0].state.terminated')) {
        return 'terminated';
    }
    ...
    return PHASE_MAP[phase] || 'unloaded';
}
